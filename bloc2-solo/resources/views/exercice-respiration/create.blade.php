...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        <h1 class="font-lora text-3xl font-semibold text-white">Nouvel exercice de respiration</h1>
    ...
    <form method="POST" action="{{ route('exercice_respiration.store') }}">
        @csrf
            ...
            <div class="bg-slate-800 border border-slate-700 rounded-2xl p-6 flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <textarea id="description" name="description" rows="4"
                              placeholder="Décrivez les bienfaits de cet exercice…"
                              class="bg-slate-900 border border-slate-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-200 placeholder-slate-500 rounded-lg px-4 py-2.5 text-sm outline-none transition resize-none">{{ old('description') }}</textarea>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="number" id="nombre_cycles" name="nombre_cycles"
                           value="{{ old('nombre_cycles', 5) }}" min="1"
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" id="duree_inspiration" name="duree_inspiration"
                               value="{{ old('duree_inspiration', 5) }}" min="1"
                               oninput="updatePreview()"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" id="duree_expiration" name="duree_expiration"
                               value="{{ old('duree_expiration', 5) }}" min="1"
                               oninput="updatePreview()"
                ...
                <div class="bg-slate-900 rounded-xl p-4 flex flex-col gap-3">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="flex flex-col items-center min-w-[52px]">
                            <span id="prev-inspire" class="text-2xl font-bold text-emerald-400 leading-none">5s</span>
                            <span class="text-[10px] uppercase tracking-wider text-emerald-600 mt-1">Inspiration</span>
                        </div>
                        <span id="sep-apnee" class="text-slate-600 text-lg pb-4" style="display:none">—</span>
                        <div id="bloc-apnee" class="flex flex-col items-center min-w-[52px]" style="display:none">
                            <span id="prev-apnee" class="text-2xl font-bold text-amber-400 leading-none">0s</span>
                            <span class="text-[10px] uppercase tracking-wider text-amber-600 mt-1">Apnée</span>
                        </div>
                        <span class="text-slate-600 text-lg pb-4">—</span>
                        <div class="flex flex-col items-center min-w-[52px]">
                            <span id="prev-expire" class="text-2xl font-bold text-blue-400 leading-none">5s</span>
                            <span class="text-[10px] uppercase tracking-wider text-blue-600 mt-1">Expiration</span>
                        </div>
                    </div>
                    <p id="preview-total" class="text-xs text-slate-500 text-center">
                        Durée totale : 50s (~1 min) pour 5 cycles
                    </p>
                </div>
